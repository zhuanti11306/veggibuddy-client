...
import charactorModel from "$assets/model/carrot/carrot.glb?url";
import floorModel from "$assets/model/floor.glb?url";
...
export function getFloorHeightAt(x: number, z: number): number {
  ...
  model.raycaster.set(rayOrigin, rayDirection);

  // 檢測
  const intersects = model.raycaster.intersectObject(floor, true);
